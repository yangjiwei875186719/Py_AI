...
class TorchRNN(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.layer = nn.RNN(input_size, hidden_size, bias=False, batch_first=True)  # input_size输入维度 hidden_size输出维度 bias 参数b batch_first当第一维.不写最后一个是
...
class DiyModel:
    ...
    def forward(self, x):
        ht = np.zeros((self.hidden_size))  # 全0 向量
        output = []
        for xt in x:
...
w_ih = torch_model.state_dict()["layer.weight_ih_l0"]   # U
w_hh = torch_model.state_dict()["layer.weight_hh_l0"]   # W
