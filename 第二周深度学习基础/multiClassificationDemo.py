...
class MultiClassficationModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.loss = nn.functional.cross_entropy  # loss函数采用交叉熵损失 自带softmax
...
def build_dataset(total_sample_num):
    ...
    for i in range(total_sample_num):
        ...
        X.append(x)
        Y.append(y)
    return torch.FloatTensor(X), torch.LongTensor(Y)  # Long是整型 ，交叉熵 torch.LongTensor(Y)
...
def main():
    ...
    for epoch in range(epoch_num):
        ...
        for batch_index in range(train_sample // batch_size):
            ...
            loss = model(x, y)  #5.1 计算loss
            loss.backward()  # 5.2计算梯度
            optim.step()  # 5.3更新权重
            optim.zero_grad()  # 5.4梯度归零
            watch_loss.append(loss.item())
        print("=========\n第%d轮平均loss:%f" % (epoch + 1, np.mean(watch_loss)))
        acc = evaluate(model)  # 5.5测试本轮模型结果
