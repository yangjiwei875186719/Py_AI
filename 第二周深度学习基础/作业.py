...
class SentimentAnalysisModel(nn.Module):             # 模型类
    def __init__(self, input_size, num_classes): # input_size #num_classes
        super(SentimentAnalysisModel, self).__init__()
        self.linear = nn.Linear(input_size, num_classes) # 线性层
        self.loss = nn.CrossEntropyLoss()  # 交叉熵          #  类加()
...
def main():
    ...
    model1 = SentimentAnalysisModel(input_size, num_classes)  # 输入值，输出值
    ...
    for epoch in range(epoch_num):
        ...
        accuracy = evaluate(model1, test_sample_num=1000)
