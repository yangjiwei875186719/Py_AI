...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        if model_type == "fast_text":
            ...
            self.use_bert = True
            self.encoder = BertModel.from_pretrained(config["pretrain_model_path"], return_dict=False)  # from_pretrained加载预训练好的权重
        ...
        self.classify = nn.Linear(hidden_size, class_num)   # 线性层，从768映射到多少类
    ...
    def forward(self, x, target=None):
        ...
        if isinstance(x, tuple):  #RNN类的模型会同时返回隐单元向量，我们只取序列结果,适配多种输出方式，只取第一个位置数
            x = x[0]
        #可以采用pooling的方式得到句向量
