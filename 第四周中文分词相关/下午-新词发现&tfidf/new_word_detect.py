...
class NewWordDetect:
        ...
        with open(path, encoding="utf8") as f:
            for line in f:
                ...
                for word_length in range(1, self.max_word_length):
                    self.ngram_count(sentence, word_length)  # ngram_count数词频 例如：2个字符相邻的字符
        return
    ...
    def ngram_count(self, sentence, word_length):
        for i in range(len(sentence) - word_length + 1):
            ...
            if i - 1 >= 0:
                char = sentence[i - 1]
                self.left_neighbor[word][char] = self.left_neighbor[word].get(char, 0) + 1  # 左邻
            if i + word_length < len(sentence):
                char = sentence[i +word_length]
                self.right_neighbor[word][char] = self.right_neighbor[word].get(char, 0) + 1  # 右邻
        return

    #计算熵
    def calc_entropy_by_word_count_dict(self, word_count_dict):
        total = sum(word_count_dict.values())  # 取出总次数
    ...
    #计算互信息(pointwise mutual information) 内部凝固度
    def calc_pmi(self):
...
# 综合打分
    def calc_word_values(self):
        ...
        for word in self.pmi:
            ...
            self.word_values[word] = pmi * min(le, re)  #  左熵* 右熵 * 内部凝固度 ，低分，高分会拉高分值所以用  pmi * min(le, re)
...
if __name__ == "__main__":
    ...
    print([x for x, c in value_sort if len(x) == 3][:10])  # 3字词
